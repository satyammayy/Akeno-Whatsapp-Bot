...
const { RemoveBgError, RemoveBgResult, removeBackgroundFromImageBase64, removeBackgroundFromImageFile } = require('remove.bg') 
...
const Nsfw = require('./lib/nsfw.js')
...
module.exports = msgHandler = async (client, message) => {
	...
	const mess = {
      error: { St: "[❗] Write *!sticker* either in the caption of an image/gif or reply to an image/gif with the command." } 
				};
	...
	if (isGroupMsg && isRule && (type === 'chat' && message.body.includes('chat.whatsapp.com') && isBotGroupAdmins) && !isGroupAdmins) return await client.removeParticipant(chat.id, author)
		...
		const miranda = ['https://media.giphy.com/media/uomDFV7IcK2Vq/giphy.mp4','https://media.giphy.com/media/vlmF0NDM01rlmTTmxf/giphy.mp4','https://media.giphy.com/media/NqB417zzxGArjvRZ48/giphy.mp4','https://media.giphy.com/media/a7MWSPaLGJziroFVWO/giphy.mp4','https://media.giphy.com/media/3PTBGM86pjGDdcZ9pc/giphy.mp4','https://media.giphy.com/media/y5yDGSHC0GQPqqM3QZ/giphy.mp4','https://media.giphy.com/media/jF8xp1wf2IE7BAdCzI/giphy.mp4','https://media.giphy.com/media/aJO5tu3YsfyvXinHQf/giphy.mp4','https://media.giphy.com/media/GyMLaYZ6EJUlU1fvG5/giphy.mp4','https://media.giphy.com/media/Ktb3t5iJAm9eygJBT6/giphy.mp4','https://media.giphy.com/media/MsBYBD74QtXjrxLL5T/giphy.mp4']
        ...
        case 'wh':
                fun.wholesome()
                    .then(async (body) => {
                        ...
                        await client.sendStickerfromUrl(from, wholesomegx, null, { author: 'Akeno', pack: 'Stickers' })
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await client.reply(from, 'Sorry Senpai, something went wrong!!', id)
...
${desc}`)
        ...
        case 'bc':
            ...
            for (let ids of chatz) {
                ...
                if (!cvk.isReadOnly) client.sendText(ids, `[ Akeno BOT Broadcast ]\n\n${msg}`)
            }
        ...
        case 'kickall':
            ...
            client.reply(from, 'Success removing all members', id)
            break
...
*Link*: ${url}`
		...
		case 'slap':
			client.sendStickerfromUrl(from, (await neko.sfw.slap()).url, null, { author: 'Akeno', pack: 'Stickers', keepScale: true });
	...
	//end of nsfw commands
		...
		case 'wasted':
                if (isMedia && type === 'image' || quotedMsg && quotedMsg.type === "image") {
                    ...
                    await client.sendFileFromUrl(from, `https://some-random-api.ml/canvas/wasted?avatar=${fotoWtNya}`, 'Wasted.jpg', 'Sticker is coming, Senpai.... just little more', id).then(() => client.sendStickerfromUrl(from, `https://some-random-api.ml/canvas/wasted?avatar=${fotoWtNya}`))
